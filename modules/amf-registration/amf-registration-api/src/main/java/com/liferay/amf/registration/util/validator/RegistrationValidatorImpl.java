...
public class RegistrationValidatorImpl implements RegistrationValidator{
	...
	public void validate(String firstName, String lastName, String email, String userName, boolean male, int bMonth, int bDay, int bYear, 
		...
		if (isRegistrationValid(firstName, lastName, email, userName, male, bMonth, bDay, bYear, password1,
	...
	public boolean isRegistrationValid(String firstName, String lastName, String email, String userName, boolean male, int bMonth, int bDay, int bYear, 
			String password1, String password2, String homePhone, String mobilePhone, String address1, String address2, String city,
			String zip, String securityQuestion, String securityAnswer, boolean acceptedTou, List<String> errors) {
		
		validateBasicInfo(userName, email, firstName, lastName, bMonth, bDay, bYear, password1, password2, errors);
	...
	private void validateBasicInfo(String userName, String email, 
			String firstName, String lastName, int bMonth, int bDay, 
			int bYear, String password1, String password2, List<String> errors){
		...
		email = email.replace(".","");
		if(!pEmail.matcher(email).matches()) errors.add("emailErrorNonAlpha");
		
		if(!p.matcher(userName).matches()) errors.add("userNameErrorNonAlpha");
		if(userName.length() < 4 || userName.length() > 16) errors.add("userNameErrorLength");
		if(userLocalService.fetchUserByScreenName(20101, userName) != null) errors.add("userNameErrorNotUnique");
	...
	private boolean compareBirthday(int bDay, int bMonth, int bYear) {
		...
		if(todaysDate.compareTo(birthDate) < 0) return true;
		return false;
	}
	...
	private void validateBillingAddress(String address1, String address2, String city, String zip, List<String> errors){
		Pattern p = Pattern.compile("^[a-zA-Z0-9]*$");
		Pattern pNumber = Pattern.compile("^[0-9]{5}$");
		
		if(address1.length() > 255) errors.add("address1ErrorTooLong");
		if(address1.equals("")) errors.add("address1ErrorEmpty");
		address1 = address1.replace(" ", "");
		if(!p.matcher(address1).matches()) errors.add("address1ErrorNonAlpha");
		
		if(address2.length() > 255) errors.add("address2ErrorTooLong");
		address2 = address2.replace(" ", "");
		if(!p.matcher(address2).matches()) errors.add("address2ErrorNonAlpha");
		
		if(!p.matcher(city).matches()) errors.add("cityErrorNonAlpha");
		if(city.length() > 255) errors.add("cityErrorTooLong");
		
		if(!pNumber.matcher(zip).matches()) errors.add("zipError");
	}
	
	private void validateMisc(String securityAnswer, boolean acceptedTou, List<String> errors) {
		
		Pattern p = Pattern.compile("^[a-zA-Z0-9]*$");
		
		if(!acceptedTou) errors.add("acceptedTouError");
		
		if(!p.matcher(securityAnswer).matches()) errors.add("securityAnswerNonAlpha");
		if(securityAnswer.length() > 255) errors.add("secuirtyAnswerTooLong");
		if(securityAnswer.equals("")) errors.add("securityAnswerEmpty");
	}
